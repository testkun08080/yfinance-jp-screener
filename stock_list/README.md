...
`sumalize.py` で算出する指標のうち、**ネットキャッシュ**と**ネットキャッシュ比率**は次のとおりです。

| 指標                 | 計算式                                                                        |
| -------------------- | ----------------------------------------------------------------------------- |
| ネットキャッシュ     | 流動資産 +（投資有価証券 × 0.7）− 負債 ※投資有価証券は未取得時は 0 として扱う |
| ネットキャッシュ比率 | ネットキャッシュ ÷ 時価総額                                                   |

**どの時点のデータか**

- **分子（流動資産・投資有価証券・負債など）**  
  yfinance の `balance_sheet` から、**最新の決算期に対応する列（先頭列）**のみを参照しています。CSV の「決算月」は、その期の日付を表します。
- **分母（時価総額）**  
  `ticker.info` の **`marketCap`** です。yfinance が返す、**スクリプト実行時に近い時点の時価総額**（実質は直近の株価と発行済株式数に基づく値）であり、**決算日の株価や決算発表日に合わせて補正はしていません**。

そのため、ネットキャッシュ比率は **「最新公表のバランスシート」** と **「取得実行時点の時価総額」** を組み合わせた値です。会計上の期末と市場価格の基準日が一致しない点に留意してください。

**その他の株価利用**

- **PER（前年度）** などは、過去の決算期末に近い日の終値を `ticker.history` で取得する処理があります。これはネットキャッシュ比率の計算とは別用途です。
...
- Part 1 から開始し、stocks*\*.json / us_stocks*\*.json の数に応じて Part 2, 3, … が自動連鎖（各パート約 1000 社ずつ）
