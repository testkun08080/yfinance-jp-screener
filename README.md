...
そんな思いと、わが投資術を参考にして作成した**個人開発による実験的プロジェクト**です。
...
2. 上記の**Workflow permissions設定を完了**します（重要）
3. フォークしたプライベートリポジトリの **Actions** タブに移動
4. **"📊 Stock Data Fetch"** ワークフローを選択
5. **"Run workflow"** をクリック
6. 処理対象の株式ファイルを選択（stocks_1.json ～ stocks_4.json）
7. 実行完了後、`stock_list/Export/` ディレクトリにデータファイルが生成されます

### ⚠️ 重要事項

- **Workflow permissions設定は必須**: 設定しないとデータファイルの保存に失敗します
